...
class DPT(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        Args:
            ...
            return_features: If True, return intermediate layer_rn features for RAFT-Stereo

        Returns:
            Dict[str, Tensor] or List[Tensor] if return_features=True
        """
        B, S, N, C = feats[0][0].shape
        feats = [feat[0].reshape(B * S, N, C) for feat in feats]

        # For RAFT-Stereo: return intermediate features (layer_rn)
        if return_features:
            layer_rns = self._forward_features(feats, H, W, patch_start_idx)
            # Reshape back: [B*S, C, H, W] -> [B, S, C, H, W] -> squeeze S if S=1
            if S == 1:
                return [lr.view(B, S, *lr.shape[1:]).squeeze(1) for lr in layer_rns]
            else:
                return [lr.view(B, S, *lr.shape[1:]) for lr in layer_rns]
    ...
    # -------------------------------------------------------------------------
    # Forward features for RAFT-Stereo (returns layer_rn)
    # -------------------------------------------------------------------------
    def _forward_features(
        ...
        """
        Extract intermediate layer_rn features for RAFT-Stereo.
        Similar to _forward_impl but returns layer_rn instead of depth prediction.

        Returns:
            List of 4 layer_rn feature maps [layer_1_rn, layer_2_rn, layer_3_rn, layer_4_rn]
        """
        B, _, C = feats[0].shape
        ph, pw = H // self.patch_size, W // self.patch_size
        resized_feats = []
        ...
        # Return layer_rn features (before fusion)
        l1, l2, l3, l4 = resized_feats
        layer_1_rn = self.scratch.layer1_rn(l1)
        layer_2_rn = self.scratch.layer2_rn(l2)
        layer_3_rn = self.scratch.layer3_rn(l3)
        layer_4_rn = self.scratch.layer4_rn(l4)

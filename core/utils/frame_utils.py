...
def readDispTartanairSynthetic(file_name, baseline, focal=320.0):
    """Read depth and convert to disparity for Tartanair Synthetic dataset.

    Args:
        file_name: path to depth .npy file
        baseline: stereo baseline in meters (0.2, 0.3, 0.4, 0.5)
        focal: focal length in pixels (default 320)
    ...
    valid = (disp > 0) & (depth > 0) & np.isfinite(disp)
    return disp, valid


def readDispSceneFlow(file_name):
    """Read SceneFlow disparity from .pfm file with valid mask like TartanAir"""
